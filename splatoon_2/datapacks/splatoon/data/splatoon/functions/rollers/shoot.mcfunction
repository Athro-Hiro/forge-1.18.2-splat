...
execute if entity @s[team=blue] anchored eyes run summon item ^ ^ ^0.1 {Item:{id:"minecraft:blue_wool",Count:1},Age:-32767,PickupDelay:32767,Invulnerable:1,Tags:[roller_bullet,bullet,can_falloff,blue]}
execute if entity @s[team=yellow] anchored eyes run summon item ^ ^ ^0.1 {Item:{id:"minecraft:yellow_wool",Count:1},Age:-32767,PickupDelay:32767,Invulnerable:1,Tags:[roller_bullet,bullet,can_falloff,yellow]}
execute if entity @s[tag=horizontal] run scoreboard players set type value 1
execute if entity @s[tag=vertical] run scoreboard players set type value 2
scoreboard players set temp value 3
scoreboard players operation range value = @s range
execute if entity @s[tag=vertical] run scoreboard players operation range value *= temp value
scoreboard players set temp value 2
execute if entity @s[tag=vertical] run scoreboard players operation range value /= temp value
scoreboard players operation damage value = @s damage
execute if entity @s[tag=horizontal,tag=!middle] run scoreboard players operation damage value /= temp value
...
execute as @e[type=item,tag=roller_bullet,tag=!timer,limit=1,sort=nearest] run function splatoon:rollers/shoot_as_item
